...
import IOSPrompt from '@/components/menu/IOSPrompt';
...
export default function MenuClient({ restaurant: initialRestaurant }: { restaurant: MenuPageRestaurant }) {
    ...
    const [activeTab, setActiveTab] = useState<'menu' | 'search' | 'info'>('menu');
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        if (touchStart !== null && window.scrollY === 0) {
            ...
            if (currentY - touchStart > 100 && !isRefreshing) {
                handleRefresh();
            }
        }
    };
    ...
    const handleRefresh = async () => {
        ...
        try {
            const res = await fetch(`/api/public/menu?slug=${restaurant.slug}`);
            if (res.ok) {
        ...
        >
            ...
            {activeTab === 'search' && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
                        <button onClick={() => setActiveTab('menu')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '5px' }}>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
                        </button>
                        <input
                            type="text"
                            placeholder="Cerca piatti o ingredienti..."
                            value={searchQuery}
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                            ...
                            <div className={styles.itemsGrid}>
                                {restaurant.categories.flatMap(cat =>
                                    cat.items.filter(item =>
                                        ...
                                        getItemDesc(item)?.toLowerCase().includes(searchQuery.toLowerCase())
                                    ).map(item => (
                                        <article key={item.id} className={`${styles.itemCard} ${getCardClass()}`} style={{ backgroundColor: 'white', marginBottom: '15px' }}>
                                            <div className={styles.itemInfo}>
                                                <div className={styles.itemHeader}>
                                                    <h3 className={styles.itemName}>{getItemName(item)}</h3>
                                                    {item.price !== null && <span className={styles.itemPrice}>€ {Number(item.price).toFixed(2)}</span>}
                                                </div>
                                                <p className={styles.itemDesc}>{getItemDesc(item)}</p>
                                            </div>
                                        </article>
                                    ))
                                )}
                        ...
                        {searchQuery.length > 0 && restaurant.categories.every(cat => !cat.items.some(item => getItemName(item).toLowerCase().includes(searchQuery.toLowerCase()) || getItemDesc(item)?.toLowerCase().includes(searchQuery.toLowerCase()))) && (
                            <p style={{ textAlign: 'center', color: '#999', marginTop: '40px' }}>Nessun risultato trovato.</p>
                        )}
                    </div>
                </div>
            )}
            ...
            {activeTab === 'info' && (
                <div style={{
                    ...
                    <button onClick={() => setActiveTab('menu')} style={{ position: 'absolute', top: '20px', left: '20px', background: 'none', border: 'none', cursor: 'pointer', padding: '5px' }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>

                    <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                        {restaurant.logoUrl && <img src={restaurant.logoUrl} alt={restaurant.name} style={{ width: '80px', height: '80px', borderRadius: '50%', marginBottom: '15px', objectFit: 'cover' }} />}
                        <h2 style={{ fontSize: '1.8rem', fontWeight: '800' }}>{restaurant.name}</h2>
                        <p style={{ color: '#666', marginTop: '10px' }}>{getRestaurantDesc()}</p>
                    </div>

                    <div style={{ width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {restaurant.whatsappNumber && (
                            <a href={`https://wa.me/${restaurant.whatsappNumber}`} target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '15px', background: '#f0fdf4', color: '#166534', borderRadius: '16px', textDecoration: 'none', fontWeight: '600' }}>
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" /></svg>
                                Contattaci su WhatsApp
                            </a>
                        )}
                        <div style={{ marginTop: '20px', padding: '20px', background: '#f8f9fa', borderRadius: '16px', fontSize: '0.9rem', color: '#666' }}>
                            <p style={{ marginBottom: '10px' }}>Powered by <strong>Gardigital.it</strong></p>
                            <p>Versione PWA 1.0.0 (Building Fundamentals)</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <footer className={styles.footer} style={{ backgroundColor: 'transparent', color: restaurant.textColor, opacity: 0.6, paddingBottom: activeTab === 'menu' ? '120px' : '40px' }}>
