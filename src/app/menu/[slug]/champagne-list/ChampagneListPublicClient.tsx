...
export default function ChampagneListPublicClient({ restaurant, champagneList }: { restaurant: Restaurant, champagneList: ChampagneList | null }) {
    ...
    // Premium Champagne List Design (Gold/Premium Theme override or mix)
    // We can use the restaurant theme but maybe add some gold accents for Champagne.
    ...
    return (
        <div style={containerStyle}>
            ...
            <main style={{ maxWidth: '800px', margin: '0 auto', padding: '1rem 1.5rem 4rem 1.5rem' }}>
                ...
                {!champagneList || !champagneList.isActive || champagneList.sections.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '4rem 1rem', opacity: 0.6 }}>
                        <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>🥂</div>
                        <p style={{ fontSize: '1.1rem' }}>La carta champagne è in fase di aggiornamento.</p>
                    </div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4rem' }}>
                        {champagneList.sections.map((section, idx) => (
